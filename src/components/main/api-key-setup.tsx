...
export default function ApiKeySetup({ onApiKeySubmit }: ApiKeySetupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[v0] Attempting API validation with key:", trimmedKey.substring(0, 10) + "...")

      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1/models?key=${encodeURIComponent(trimmedKey)}`
      )
      ...
      if (!response.ok) {
        const errorMessage = responseData?.error?.message || "Invalid API key"
      ...
      console.log("[v0] Calling onApiKeySubmit with key:", trimmedKey.substring(0, 10) + "...")
      onApiKeySubmit(trimmedKey)
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Invalid API key. Please check and try again."
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-950 via-slate-900 to-slate-950 flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-2xl border-blue-500/20 bg-slate-800/80 backdrop-blur">
        <CardHeader className="space-y-2">
          <CardTitle className="text-3xl text-white font-bold">ExcaliDraw AI</CardTitle>
          <CardDescription className="text-slate-300">Bring Your Own Gemini API Key</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && (
              <div className="p-3 bg-red-900/30 border border-red-500/50 rounded text-red-300 text-sm">{error}</div>
            )}
